...
const transporter = nodemailer.createTransport({
  ...
  auth: {
      ...
      pass: 'rjko mrtk aqtl jeny' // Your email password or app-specific password
  }
});
...
export const updateStatus = async (req, res) => {
  ...
  try {
      ...
      else if(complaint.status=="Accepted" && req.body._otp){
        const _otp = req.body.otp;
        // await sendOTP(complaint.email);
...
const authenticate = async (_otp) => {
  ...
  const complaint = await Complaint.findOne({ 'otp.code': _otp });
  
  if(complaint){
      // console.log(student.otp);
      if (!complaint.otp || complaint.otp.code !== _otp || new Date() - complaint.otp.createdAt > 5 * 60 * 1000) {
          await complaint.deleteOne();
