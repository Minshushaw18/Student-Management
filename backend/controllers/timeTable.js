...
export const getTimetable = async (req, res) => {
  try {
      ...
      const semester = req.body.semester;// Extract the semester value from the request body
...
export const createTimetable = async (req, res) => {
    try {
        const staffID = req.params.id;
        const { program, branch, batch, semester, days } = req.body;
...
export const updateTimetable = async (req, res) => {
    ...
    try {
        ...
        const updatedTimetable = await Timetable.findByIdAndUpdate(timetableID, req.body, { new: true });  // Update and return updated document
        if (!updatedTimetable) {
            return res.status(404).json({ message: "Timetable not found!" });
        }
